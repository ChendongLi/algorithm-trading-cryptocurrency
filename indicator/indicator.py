...
def on_balance_volume(data, trend_periods=21, close_col='<close>', vol_col='<VOL>'):
    ...
    for index, row in data.iterrows():
        ...
        data.loc[index, 'obv'] = current_obv
...
def price_volume_trend(data, trend_periods=21, close_col='<close>', vol_col='<VOL>'):
    ...
    for index, row in data.iterrows():
        ...
        data.loc[index, 'pvt'] = current_val
...
"""
def average_true_range(data, trend_periods, 
    open_col='open', high_col='high', low_col='low', close_col='close', drop_tr = True):
...
def bollinger_bands(data, trend_periods=20, close_col='<close>'):
    ...
    data['bol_bands_middle'] = data[close_col].rolling(trend_periods, min_periods =0).mean()
    bol_std = data[close_col].rolling(trend_periods, min_periods =0).std()
    data['bol_bands_upper'] = data['bol_bands_middle'] + 2 * bol_std
    data['bol_bands_lower'] = data['bol_bands_middle'] - 2 * bol_std
...
"""
def directional_movement_index(data, periods, high_col='high', low_col='low'
, open_col = 'open', close_col='close'):
    data.reset_index(inplace = True)

    remove_tr_col = False
    if not 'true_range' in data.columns:
        data = average_true_range(data, periods, open_col= open_col
        , high_col= high_col, low_col= low_col, close_col= close_col, drop_tr = False)
        remove_tr_col = True
    ...
    for i,row in data.iterrows():
        if i>0:
            data.at[i, 'm_plus'] = row[high_col] - data.loc[i-1, high_col]
            data.at[i, 'm_minus'] = row[low_col] - data.loc[i-1, low_col]
    ...
    #data['di_plus'] = (data['dm_plus'] / data['true_range']).rolling(window=periods, min_periods=0).mean()
    #data['di_minus'] = (data['dm_minus'] / data['true_range']).rolling(window=periods, min_periods=0).mean()
...
def keltner_channel(data, periods =14, multiplier = 2
    ...
    KelChM = pd.Series((data[close_col]).rolling(periods, min_periods=0).mean(),
                       name='KelM')

    KelChU = pd.Series((multiplier * (data[high_col] - data[low_col]) + data[close_col]).rolling(periods, min_periods=0).mean(),
                       name='KelU')

    KelChD = pd.Series((multiplier * (-data[high_col] +  data[low_col]) + data[close_col]).rolling(periods, min_periods=0).mean(),
                       name='KelD')

    data = data.join(KelChM)
    data = data.join(KelChU)
